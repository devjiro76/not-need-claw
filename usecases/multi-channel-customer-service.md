...
- Gmail access (e.g., `gog` CLI or Gmail API via an MCP server / npm package)
- Google Business Profile API for reviews
- Message routing logic in AGENTS.md

## How to Set It Up

1. **Connect channels** via your agent's configuration:
   - WhatsApp Business API (through 360dialog or official API)
   - Instagram via Meta Business Suite
   - Gmail via OAuth (using a Gmail MCP server, npm package, or direct API integration)
