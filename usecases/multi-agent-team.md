...
This use case sets up multiple AI agents as a coordinated team, each specialized in a domain, communicating through shared memory and controlled via Telegram.
...
- Telegram bot integration for the shared control interface
- Sub-agent / parallel task orchestration (e.g., running multiple agent processes, using a framework like CrewAI, AutoGen, or Claude's sub-agent API)
- Shared file system or database for team memory (e.g., a shared directory, SQLite, or a note-taking tool)
...
> **Implementation note**: The routing can be implemented with a lightweight dispatcher script that watches the Telegram group for messages and forwards them to the appropriate agent process. Each agent runs as a separate process (or container) with its own system prompt and context.
