...
No special skill installation required. Set up the individual components:

- **Polymarket API** — Use `web_fetch` / `curl` / a web scraping MCP server to pull market data from the [Polymarket API](https://docs.polymarket.com/)
- **Database** — PostgreSQL, SQLite, or even a local JSON/CSV file for trade logs and portfolio tracking
- **Discord integration** — A Discord webhook or bot for daily report delivery
- **Cron / scheduling** — Use cron jobs, a scheduler, or the agent's built-in scheduling for continuous monitoring
- **Parallel analysis** — Use sub-agents or parallel tasks for analyzing multiple markets simultaneously
