...
1. **Main agent receives task** -> spawns a sub-agent with specific scope
2. **Sub-agent reads STATE.yaml** -> finds its assigned tasks
3. **Sub-agent works autonomously** -> updates STATE.yaml on progress
...
- **Sub-agent / parallel task execution**: Use your AI framework's built-in sub-agent or task spawning (e.g., Claude Code's `Task` tool, LangGraph sub-graphs, or shell-based background processes)
...
3. If PM exists -> send task to the existing sub-agent
4. If new project -> spawn a new sub-agent with the task
...
2. Spawns a sub-agent (label: "pm-auth-refactor")
   with task: "Refactor auth module, update docs. Track in STATE.yaml"
