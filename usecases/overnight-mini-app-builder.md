...
This workflow turns your AI agent into a self-directed employee. You brain dump your goals once, and the agent autonomously generates, schedules, and completes tasks that advance those goals — including building you surprise mini-apps overnight.

## What It Does

- You brain dump all your goals, missions, and objectives into the AI agent (personal and professional)
...
- Sub-agent / parallel task orchestration for autonomous task execution
- Next.js or similar (for the Kanban board — the AI agent builds it for you)

## How to Set It Up

### Step 1: Brain Dump Your Goals

This is the most important step. Text your AI agent everything you're trying to accomplish:
...
### Race Condition: Sub-Agents Editing Shared Files
...
**Why it happens:** The agent's `edit` tool requires an exact `oldText` match. If a sub-agent updates a line between the time your main session reads the file and tries to edit it, the text no longer matches — the edit silently fails.
...
This pattern is borrowed from Git's commit log: you never rewrite history, you only add new commits. It eliminates race conditions entirely and has a bonus: `AUTONOMOUS.md` stays small, so it costs fewer tokens every time it's loaded in a scheduled check.
...
The task tracking file gets loaded on every scheduled poll. If it grows unbounded with completed tasks, you'll burn tokens unnecessarily.
...
Inspired by [Alex Finn](https://www.youtube.com/watch?v=UTCi_q6iuCM&t=414s) and his [video on life-changing AI agent use cases](https://www.youtube.com/watch?v=41_TNGDDnfQ).

## Related Links

- [Claude Code](https://github.com/anthropics/claude-code) — AI coding agent
- [OpenAI Codex](https://openai.com/index/openai-codex/) — alternative AI agent
